...
bool Dribbler::run(int8_t a, QVector2D obstacle)
{
    ...
    qDebug() << traveled << a << ang_final << (PO.distanceToPoint(zero) < radius * 1.2);
    qDebug() << (ang > (M_PI*(1 - (1/12))));
    qDebug() << (M_PI - (M_PI/12));
    if(traveled && ang_final && (PO.distanceToPoint(zero) < radius * 1.2)){
        qDebug() << "Retornou";
        qDebug() << "*************************************";
        ...
        go(dribbler, obstacle + QVector2D(qCos(ang + a*ang_pp), qSin(ang + a*ang_pp)) * radius);
    }
    else{
        go(dribbler, obstacle + QVector2D(qCos(ang - a*ang_pp), qSin(ang - a*ang_pp)) * radius);
    }
